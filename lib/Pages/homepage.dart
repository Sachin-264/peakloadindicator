...
class _HomePageState extends State<HomePage>
  ...
  static final DateTime _appStartTime =
  DateTime.now(); // Used for Uptime calculation
  ...
  void initState() {
    ...
    _pages = [
      ...
      OpenFilePage(
        // <-- I replaced the Placeholder with this
  ...
  void _startAutoStartCheck() {
    ...
    _autoStartTimer =
        Timer.periodic(const Duration(seconds: 30), (timer) async {
          ...
          final startTimeHr =
              (autoStartData['StartTimeHr'] as num?)?.toDouble() ?? 0.0;
          final startTimeMin =
              (autoStartData['StartTimeMin'] as num?)?.toDouble() ?? 0.0;
          final endTimeHr = (autoStartData['EndTimeHr'] as num?)?.toDouble() ?? 0.0;
          final endTimeMin =
              (autoStartData['EndTimeMin'] as num?)?.toDouble() ?? 0.0;
          final scanTimeSec =
              (autoStartData['ScanTimeSec'] as num?)?.toDouble() ?? 1.0;

          final now = DateTime.now();
          final currentHr = now.hour.toDouble();
          final currentMin = now.minute.toDouble();

          if (currentHr == startTimeHr.floor() &&
              currentMin == startTimeMin.floor()) {
            LogPage.addLog(
                '[HomePage] Time match found for AutoStart, fetching channels');
                    ...
                    content: Text(
                      ...
                      style: GoogleFonts.poppins(
                          color: ThemeColors.getColor(
                              'sidebarText', Global.isDarkMode.value)),
                    ),
            ...
            final startTime = DateTime(
                now.year, now.month, now.day, startTimeHr.toInt(), startTimeMin.toInt());
            var endTime = DateTime(
                now.year, now.month, now.day, endTimeHr.toInt(), endTimeMin.toInt());
            ...
            LogPage.addLog(
                '[HomePage] AutoStart triggered: Switching to AutoStartScreen with duration: $testDuration');
            timer.cancel();
            if (mounted) {
              setState(() {
        ...
        return BackdropFilter(
          ...
          child: Dialog(
            ...
            backgroundColor: ThemeColors.getColor(
                'dialogBackground', Global.isDarkMode.value),
            child: Container(
              width: 400,
                    ...
                    children: [
                      Text('Backup & Restore',
                          style: GoogleFonts.poppins(
                              ...
                              color: ThemeColors.getColor(
                                  'dialogText', Global.isDarkMode.value))),
                      const SizedBox(height: 12),
                      Text('Securely manage your data',
                          style: GoogleFonts.poppins(
                              fontSize: 14,
                              color: ThemeColors.getColor(
                                  'dialogSubText', Global.isDarkMode.value))),
                          ...
                          children: [
                            _buildDialogButton(
                              ...
                              gradient: ThemeColors.getDialogButtonGradient(
                                  Global.isDarkMode.value, 'backup'),
                              isDarkMode: Global.isDarkMode.value,
                              onPressed: () async {
                                setState(() => isLoading = true);
                                final result = await _backupRestoreService
                                    .backupDatabase();
                                setState(() => isLoading = false);
                                Navigator.of(dialogContext).pop();
                                MessageUtils.showMessage(context, result,
                            ...
                            _buildDialogButton(
                              ...
                              gradient: ThemeColors.getDialogButtonGradient(
                                  Global.isDarkMode.value, 'restore'),
                              isDarkMode: Global.isDarkMode.value,
                              onPressed: () async {
                                ...
                                if (filePickerResult == null) {
                                  MessageUtils.showMessage(
                                      context, 'Restore cancelled by user.',
                                      isError: true);
                                  _logActivity(
                                      'Restore cancelled by user (file picker).');
                                  return;
                                }
                                String? filePath =
                                ...
                                if (filePath == null) {
                                  MessageUtils.showMessage(context,
                                      ...
                                      isError: true);
                                  _logActivity(
                                      'Restore failed: Selected file path is null.');
                                  return;
                                }
                                setState(() => isLoading = true);
                                final result = await _backupRestoreService
        ...
        return StatefulBuilder(
          builder: (context, setState) {
            ...
            void handleModeSelection(String mode) {
              setState(() {
                Global.selectedMode.value = mode;
              });
              Future.delayed(const Duration(milliseconds: 300), () {
                if (mounted) {
                  Navigator.of(context).pop();
                  MessageUtils.showMessage(context, '$mode mode selected!');
                  _logActivity('$mode mode selected');
                }
              });
            }
                ...
                child: Container(
                  ...
                  child: Column(
                    ...
                    children: [
                      ...
                      const SizedBox(height: 12),
                      Text('Choose your preferred visualization',
                          style: GoogleFonts.poppins(
                              fontSize: 14,
                              color: ThemeColors.getColor(
                      ...
                      Column(
                        children: [
                          _buildDialogButton(
                            ...
                            gradient: ThemeColors.getDialogButtonGradient(
                                isDarkMode, 'graph'),
                            isSelected: Global.selectedMode.value == 'Graph',
                            isDarkMode: isDarkMode,
                            onPressed: () => handleModeSelection('Graph'),
                          ),
                          ...
                          _buildDialogButton(
                            ...
                            gradient: ThemeColors.getDialogButtonGradient(
                                isDarkMode, 'table'),
                            isSelected: Global.selectedMode.value == 'Table',
                            isDarkMode: isDarkMode,
                            onPressed: () => handleModeSelection('Table'),
                          ),
                          const SizedBox(height: 16),
                          _buildDialogButton(
                            text: 'Combined Mode',
                            ...
                            isDarkMode: isDarkMode,
                            onPressed: () => handleModeSelection('Combined'),
                          ),
                        ],
                      ),
                      const SizedBox(height: 24),
  ...
  IconData _getModeIcon() {
    switch (Global.selectedMode.value) {
      case 'Graph':
                        ...
                        child: ValueListenableBuilder<bool>(
                          ...
                          builder: (context, isExpanded, child) {
                            return AnimatedContainer(
                              ...
                              decoration: BoxDecoration(
                                gradient: ThemeColors.getSidebarGradient(
                                    isDarkMode),
                                ...
                                SidebarLogo(
                                    onTap: () async {
                                      ...
                                      _animationController.forward(
                                          from: 0.0);
                                      _logActivity(
                                          'Navigated to Dashboard via Logo');
                                    },
                                    isSidebarExpanded:
                                    _isSidebarExpanded,
                                    isDarkMode: isDarkMode),
                                ...
                                SidebarButton(
                                    ...
                                    onTap: () {
                                      ...
                                      _animationController.forward(
                                          from: 0.0);
                                      _logActivity(
                                          'Toggled theme to ${isDarkMode ? 'Light' : 'Dark'} mode');
                                    },
                                    isSidebarExpanded:
                                    _isSidebarExpanded,
                                    isDarkMode: isDarkMode),
...
// UPDATED DIALOG BUTTON with a more vibrant light mode color
  Widget _buildDialogButton({
    ...
    // Define the visually distinct gradients for the selected state
    final LinearGradient selectedGradient = isDarkMode
        ? LinearGradient(
      colors: [Colors.cyan.shade400, Colors.teal.shade500], // Dark mode: Vibrant Cyan
      begin: Alignment.topLeft,
      end: Alignment.bottomRight,
    )
        : LinearGradient(
      colors: [Colors.purple.shade500, Colors.deepPurple.shade600], // Light mode: Vibrant Purple
      begin: Alignment.topLeft,
      end: Alignment.bottomRight,
    );

    // Define matching shadow colors for the glow effect
    final Color selectedShadowColor = isDarkMode
    ...
    return GestureDetector(
      ...
      child: AnimatedContainer(
        ...
        decoration: BoxDecoration(
          // Use the appropriate gradient based on the selection state
          gradient: isSelected ? selectedGradient : gradient,
          borderRadius: BorderRadius.circular(12),
          border: Border.all(
            // Add a subtle border to the selected button for better definition
            color: isSelected
          ...
          boxShadow: [
            // Apply the glow effect using the shadow
            BoxShadow(
              color: isSelected
                  ? selectedShadowColor
          ...
          children: [
            Text('Dashboard',
                style: GoogleFonts.poppins(
                    ...
                    fontWeight: FontWeight.w700,
                    color:
                    ThemeColors.getColor('dialogText', isDarkMode))),
            const SizedBox(height: 8),
            Text('Monitor and control your system',
                style: GoogleFonts.poppins(
                    fontSize: 16,
                    color: ThemeColors.getColor(
                        'dialogSubText', isDarkMode))),
              ...
              children: [
                _buildDashboardCard(
                    ...
                    content: Column(
                        ...
                        children: [
                          _buildStatusRow(
                              'Active Channels', '$_activeChannels', isDarkMode),
                          ...
                          LinearProgressIndicator(
                              value: _activeChannels > 0
                                  ? _activeChannels / 50.0
                                  : 0.0,
                              backgroundColor:
                ...
                _buildDashboardCard(
                    ...
                    content: Column(
                      crossAxisAlignment: CrossAxisAlignment.start,
                      children: [
                        _buildStatusRow('Version', '1.0.0', isDarkMode),
                        _buildStatusRow(
                            'Theme', isDarkMode ? 'Dark' : 'Light', isDarkMode),
                        const Spacer(),
                        Text('Last Updated: 05/26/2025',
                            style: GoogleFonts.poppins(
                                fontSize: 12,
                                color: ThemeColors.getColor(
                                    'cardText', isDarkMode)
  ...
  Widget _buildDashboardCard(
    ...
    return AnimatedContainer(
      ...
      decoration: BoxDecoration(
          ...
          border: Border.all(
              color: ThemeColors.getColor('cardBorder', isDarkMode), width: 1)),
      child: Column(
        crossAxisAlignment: CrossAxisAlignment.start,
        children: [
          Row(
...
class _SidebarButtonState extends State<SidebarButton>
  ...
  void initState() {
    ...
    _glowAnimation = Tween<double>(begin: 0.0, end: 0.3)
              ...
              child: AnimatedContainer(
                ...
                decoration: BoxDecoration(
                  color: widget.isSelected
                      ? ThemeColors.getColor(
                      'sidebarIconSelected', widget.isDarkMode)
                      .withOpacity(0.2)
                      : _isHovered
                      ? ThemeColors.getColor(
                      'sidebarGlow', widget.isDarkMode)
                      .withOpacity(0.1)
                      : Colors.transparent,
                  borderRadius: BorderRadius.circular(12),
                  boxShadow: [
                    ...
                    ScaleTransition(
                        ...
                        child: Icon(widget.icon,
                            color: widget.isSelected
                                ...
                                : _isHovered
                                ? ThemeColors.getColor(
                                'sidebarText', widget.isDarkMode)
                                : ThemeColors.getColor(
                                'sidebarIcon', widget.isDarkMode),
                            size: 24)),
                    AnimatedSlide(
                      offset:
                      isExpanded ? Offset.zero : const Offset(0.2, 0),
                      duration: const Duration(milliseconds: 300),
                      curve: Curves.easeInOut,
...
class _SidebarLogoState extends State<SidebarLogo>
    with SingleTickerProviderStateMixin {
  late AnimationController _controller;
  late Animation<double> _scaleAnimation;
  late Animation<double> _glowAnimation;
        ...
        child: ValueListenableBuilder<bool>(
          ...
          builder: (context, isExpanded, child) {
            return AnimatedContainer(
              ...
              decoration: BoxDecoration(
                color: _isHovered
                    ? ThemeColors.getColor('sidebarGlow', widget.isDarkMode)
                    .withOpacity(0.2)
                    : Colors.transparent,
                borderRadius: BorderRadius.circular(12),
                boxShadow: [
...
class AnimatedLoadingIndicator extends StatefulWidget {
  final bool isDarkMode;
  const AnimatedLoadingIndicator({Key? key, required this.isDarkMode})
      : super(key: key);
  @override
...
class _AnimatedLoadingIndicatorState extends State<AnimatedLoadingIndicator>
  ...
  void initState() {
    ...
    _colorAnimation = ColorTween(
        begin: ThemeColors.getColor(
            'buttonGradientStart', widget.isDarkMode),
          ...
          children: [
            SizedBox(
                ...
                child: CircularProgressIndicator(
                    valueColor: _colorAnimation, strokeWidth: 4)),
            ScaleTransition(
                scale: _scaleAnimation,
                child: RotationTransition(
                    turns: _rotationAnimation,
                    child: Icon(LucideIcons.hardDrive,
                        size: 40, color: _colorAnimation.value))),
          ],
        );
      },
    );

...
class _SerialPortScreenState extends State<SerialPortScreen> {
  ...
  void _updateGraphData(Map<String, dynamic> newData) {
    ...
    print('[SerialPortScreen] Sent graph data update: ${dataByChannel.keys}');
  }
  ...
  void _autoStartScan() {
    if (!isScanning && !isCancelled && !isManuallyStopped &&
      ...
      try {
        debugPrint('Autoreconnect: Attempt ${_reconnectAttempts +
            1}/$_maxReconnectAttempts');
  ...
  Future<void> _saveData() async {
    try {
      debugPrint('Saving data to database started...');
      ...
      final dbPath = path.join(appDocumentsDir.path, 'serial_port_data.db');
      ...
      debugPrint('Inserted into Test table: $testPayload');

      for (var entry in test1Payload) {
        await database.insert(
            'Test1', entry, conflictAlgorithm: ConflictAlgorithm.replace);
      }
      debugPrint('Inserted ${test1Payload.length} entries into Test1 table');

      await database.insert(
          'Test2', test2Payload, conflictAlgorithm: ConflictAlgorithm.replace);
      debugPrint('Inserted into Test2 table: $test2Payload');
  ...
  Map<String, dynamic> _prepareTestPayload(int recNo) {
    return {
      ...
      "MaxYAxis": channelConfigs.isNotEmpty ? channelConfigs.values.first
          .chartMaximumValue : 100.0,
      "MinYAxis": channelConfigs.isNotEmpty ? channelConfigs.values.first
          .chartMinimumValue : 0.0,
    };
  }

  List<Map<String, dynamic>> _prepareTest1Payload(int recNo) {
    List<Map<String, dynamic>> payload = [];
    final sortedChannels = channelConfigs.keys.toList()
      ..sort();
    ...
    for (int i = 0; i < timestamps.length; i++) {
      ...
      final data = dataByChannel[sortedChannels.first]?.firstWhere((
      ...
      Map<String, dynamic> payloadEntry = {
        ...
        "AbsDate": "${dateTime.year}-${dateTime.month.toString().padLeft(
            2, '0')}-${dateTime.day.toString().padLeft(2, '0')}",
        "AbsTime": "${dateTime.hour.toString().padLeft(2, '0')}:${dateTime
            .minute.toString().padLeft(2, '0')}:${dateTime.second.toString()
            .padLeft(2, '0')}",
        "AbsDateTime": "${dateTime.year}-${dateTime.month.toString().padLeft(
            2, '0')}-${dateTime.day.toString().padLeft(2, '0')} ${dateTime.hour
            .toString().padLeft(2, '0')}:${dateTime.minute.toString().padLeft(
            2, '0')}:${dateTime.second.toString().padLeft(2, '0')}",
        "Shown": i % 2 == 0 ? "Y" : "N",
        "AbsAvg": 0.0,
      };
      ...
      for (int j = 0; j < sortedChannels.length && j < 50; j++) {
        ...
        if (channelData['Value'] != null &&
            (channelData['Value'] as double).isFinite) {
    ...
    debugPrint(
        '[SERIAL_PORT] Prepared Test1 payload with ${payload.length} entries');
    return payload;
  }
  ...
  Map<String, dynamic> _prepareTest2Payload(int recNo) {
    ...
    for (int i = 1; i <= 50; i++) {
      String channelName = i <= sortedChannels.length
          ? channelConfigs[sortedChannels[i - 1]]!.channelName
          : '';
      payload["ChannelName$i"] = channelName;
    }

    debugPrint('[SERIAL_PORT] Prepared Test2 payload with ${sortedChannels
  ...
  Widget _buildGraph() {
    ...
    for (var channel in channelsToPlot) {
      if (!channelConfigs.containsKey(channel) ||
          !channelColors.containsKey(channel)) {
      ...
      double segmentStartTimeMs = channelData.isNotEmpty
          ? (channelData.first['Timestamp'] as num?)?.toDouble() ?? DateTime
          .now()
    ...
    if (lineBarsData.isEmpty || minX == double.infinity ||
        maxX == -double.infinity) {
      minX = DateTime
          .now()
          .millisecondsSinceEpoch
          .toDouble() - segmentDurationMs;
      maxX = DateTime
          .now()
          .millisecondsSinceEpoch
          .toDouble();
    ...
    List<double> sortedTimestamps = uniqueTimestamps.toList()
      ..sort();
    // debugPrint('[GRAPH] Unique timestamps for x-axis: $sortedTimestamps');
    ...
    Widget legend = Wrap(
      ...
      children: channelsToPlot
          .where((channel) =>
      channelConfigs.containsKey(channel) && channelColors.containsKey(channel))
          .map((channel) {
        final color = channelColors[channel];
    ...
    return Column(
      children: [
        ...
        Padding(padding: const EdgeInsets.symmetric(
            horizontal: 16.0, vertical: 8.0), child: legend),
        Expanded(
          child: Padding(
            ...
            child: LineChart(
              LineChartData(
                  ...
                  touchTooltipData: LineTouchTooltipData(
                    getTooltipItems: (touchedSpots) =>
                        touchedSpots.map((spot) {
                          ...
                          final channelName = channelConfigs[channel]
                              ?.channelName ?? 'Unknown';
                ...
                gridData: FlGridData(
                  ...
                  sortedTimestamps.contains(value)
                      ? FlLine(
                ...
                titlesData: FlTitlesData(
                  ...
                  leftTitles: AxisTitles(
                    axisNameWidget: Text(
                      'Load (${channelConfigs.isNotEmpty ? channelConfigs.values
                          .first.unit : "Unit"})',
                    ...
                    sideTitles: SideTitles(
                      ...
                      getTitlesWidget: (value, _) =>
                          Text(
                            ...
                            style: GoogleFonts.roboto(color: AppColors
                                .textPrimary, fontSize: 12),
                          ),
                    ),
                  ),
                        ...
                        return Padding(
                          ...
                          child: Text(
                            '${dateTime.hour.toString().padLeft(
                                2, '0')}:${dateTime.minute.toString().padLeft(
                                2, '0')}:${dateTime.second.toString().padLeft(
                                2, '0')}',
                            style: GoogleFonts.roboto(
